...
const (
	RaftService_AppendEntries_FullMethodName   = "/raftlite.RaftService/AppendEntries"
	RaftService_RequestVote_FullMethodName     = "/raftlite.RaftService/RequestVote"
	RaftService_InstallSnapshot_FullMethodName = "/raftlite.RaftService/InstallSnapshot"
)
...
var RaftService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "raftlite.RaftService",
...
const (
	LoopbackService_AddBlock_FullMethodName = "/raftlite.LoopbackService/AddBlock"
)

// LoopbackServiceClient is the client API for LoopbackService service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type LoopbackServiceClient interface {
...
func NewLoopbackServiceClient(cc grpc.ClientConnInterface) LoopbackServiceClient {
	return &loopbackServiceClient{cc}
}

func (c *loopbackServiceClient) AddBlock(ctx context.Context, in *AddBlockRequest, opts ...grpc.CallOption) (*AddBlockResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AddBlockResponse)
	err := c.cc.Invoke(ctx, LoopbackService_AddBlock_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// LoopbackServiceServer is the server API for LoopbackService service.
// All implementations must embed UnimplementedLoopbackServiceServer
// for forward compatibility.
type LoopbackServiceServer interface {
	AddBlock(context.Context, *AddBlockRequest) (*AddBlockResponse, error)
	mustEmbedUnimplementedLoopbackServiceServer()
}

// UnimplementedLoopbackServiceServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedLoopbackServiceServer struct{}

func (UnimplementedLoopbackServiceServer) AddBlock(context.Context, *AddBlockRequest) (*AddBlockResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method AddBlock not implemented")
}
func (UnimplementedLoopbackServiceServer) mustEmbedUnimplementedLoopbackServiceServer() {}
func (UnimplementedLoopbackServiceServer) testEmbeddedByValue()                         {}

// UnsafeLoopbackServiceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to LoopbackServiceServer will
// result in compilation errors.
type UnsafeLoopbackServiceServer interface {
	mustEmbedUnimplementedLoopbackServiceServer()
}

func RegisterLoopbackServiceServer(s grpc.ServiceRegistrar, srv LoopbackServiceServer) {
	// If the following call panics, it indicates UnimplementedLoopbackServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&LoopbackService_ServiceDesc, srv)
}

func _LoopbackService_AddBlock_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(AddBlockRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(LoopbackServiceServer).AddBlock(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: LoopbackService_AddBlock_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(LoopbackServiceServer).AddBlock(ctx, req.(*AddBlockRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// LoopbackService_ServiceDesc is the grpc.ServiceDesc for LoopbackService service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var LoopbackService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "raftlite.LoopbackService",
	HandlerType: (*LoopbackServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "AddBlock",
			Handler:    _LoopbackService_AddBlock_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "api/raft.proto",
